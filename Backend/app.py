...
def getAllLabels():
    ...
    try:
        labels = (
            LabelBoundary.handleRetrieveAllLabels()
        )  # Use the boundary to handle the retrieval of all labels
        if labels:
            return (
                jsonify([label.toDict() for label in labels]),
                200,
...
def violations():
    ...
    try:
        violations = (
            ViolationBoundary.handleRetrieveAllViolations()
        )  # Use the boundary to handle the retrieval of all violations
        if violations:
            return (
                jsonify([violation.toDict() for violation in violations]),
                200,
...
def addComment(post_id):
    ...
    try:
        ...
        commentEntity = CommentBoundary.handleCreateComment(
            comment
...
def deleteComment(comment_id):
    ...
    try:
        ...
        accountId = data["accountId"]
        success = CommentBoundary.handleDeleteComment(
            comment_id
        ...
        if success:
            ...
            return (
                jsonify({"error": f"Failed to delete comment with ID {comment_id}"}),
                500,
            )
    except Exception as e:
